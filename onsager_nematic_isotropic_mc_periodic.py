...
def plot_space(o_path,c,s,rods,r,Lx,Ly):
    ...
    plt.title('Rods Plot, Periodic BC\nConcentration = '+str(c)+' ,Order Parameter = '+str(s))
    plt.axis('scaled')
    
    plt.plot([0, 0], [0, Ly], color='black', linestyle="--", linewidth=0.4)
    plt.plot([0, Lx], [0, 0], color='black', linestyle="--", linewidth=0.4)
    plt.plot([Lx, Lx], [0, Ly], color='black', linestyle="--", linewidth=0.4)
    plt.plot([0, Lx], [Ly, Ly], color='black', linestyle="--", linewidth=0.4)
...
#    plt.yticks(yticks)
    ...
    plt.ylabel('y')
    plt.axis('scaled')
    plt.title('Order Parameter, Periodic BC\nConcentration = '+str(c))
...
def plot_relaxation_time(o_path,mean_angle_arr,c):
    ...
    plt.title('Relexation Time Plot, Periodic BC\nConcentration='+str(c))
...
def draw_allowed_angle(rods,new_rod,rod_l,Lx,Ly):
    ...
    r = rod_l/float(2)
    neighbor_rods = np.array([])
    
    #distances array (between current rod and other rods)
    if len(rods):
        
        # this few block add rods based on periodic boundary conditions
        periodic_neighbor_rods_x = np.array([])
        periodic_neighbor_rods_y = np.array([])
        periodic_neighbor_rods_xy = np.array([])
        
        if new_rod[0]<rod_l:
            periodic_rods_x = rods[Lx-rods[:,0]<rod_l].copy()
            periodic_rods_x[:,0] = periodic_rods_x[:,0] - Lx
        elif Lx-new_rod[0]<rod_l:
            periodic_rods_x = rods[rods[:,0]<rod_l].copy()            
            periodic_rods_x[:,0] = periodic_rods_x[:,0] + Lx
        
        try:
            if len(periodic_rods_x):
                periodic_x_dist = distance.cdist([new_rod],periodic_rods_x[:,0:2].tolist())
                mask = periodic_x_dist<rod_l
                periodic_neighbor_rods_x = periodic_rods_x[mask[0]]
        except(UnboundLocalError):
            pass
        
        if new_rod[1]<rod_l:
            periodic_rods_y = rods[Ly-rods[:,1]<rod_l].copy()
            periodic_rods_y[:,1] = periodic_rods_y[:,1] - Ly
        elif Ly-new_rod[1]<rod_l:
            periodic_rods_y = rods[rods[:,1]<rod_l].copy()
            periodic_rods_y[:,1] = periodic_rods_y[:,1] + Ly
        
        try:
            if len(periodic_rods_y):
                periodic_y_dist = distance.cdist([new_rod],periodic_rods_y[:,0:2].tolist())
                mask = periodic_y_dist<rod_l
                periodic_neighbor_rods_y = periodic_rods_y[mask[0]]
        except(UnboundLocalError):
            pass
        
        if new_rod[0]<rod_l and new_rod[1]<rod_l:
            periodic_rods_xy = rods[np.logical_and(Lx-rods[:,0]<rod_l,Ly-rods[:,1]<rod_l)].copy()
            periodic_rods_xy[:,0] = periodic_rods_xy[:,0] - Lx
            periodic_rods_xy[:,1] = periodic_rods_xy[:,1] - Ly
        elif new_rod[0]<rod_l and Ly-new_rod[1]<rod_l:
            periodic_rods_xy = rods[np.logical_and(Lx-rods[:,0]<rod_l,rods[:,1]<rod_l)].copy()
            periodic_rods_xy[:,0] = periodic_rods_xy[:,0] - Lx
            periodic_rods_xy[:,1] = periodic_rods_xy[:,1] + Ly
        elif Lx-new_rod[0]<rod_l and new_rod[1]<rod_l:
            periodic_rods_xy = rods[np.logical_and(rods[:,0]<rod_l,Ly-rods[:,1]<rod_l)].copy()
            periodic_rods_xy[:,0] = periodic_rods_xy[:,0] + Lx
            periodic_rods_xy[:,1] = periodic_rods_xy[:,1] - Ly         
        elif Lx-new_rod[0]<rod_l and Ly-new_rod[1]<rod_l:
            periodic_rods_xy = rods[np.logical_and(rods[:,0]<rod_l,rods[:,1]<rod_l)].copy()
            periodic_rods_xy[:,0] = periodic_rods_xy[:,0] + Lx
            periodic_rods_xy[:,1] = periodic_rods_xy[:,1] + Ly
        
        try:
            if len(periodic_rods_xy):
                periodic_xy_dist = distance.cdist([new_rod],periodic_rods_xy[:,0:2].tolist())
                mask = periodic_xy_dist<rod_l
                periodic_neighbor_rods_xy = periodic_rods_xy[mask[0]]
        except(UnboundLocalError):
            pass
...
#        print(dist<rod_l)
        ...
        mask = dist<rod_l
        regular_neighbor_rods = rods[mask[0]]
        
        if len(regular_neighbor_rods) + len(periodic_neighbor_rods_x) + len(periodic_neighbor_rods_y) + len(periodic_neighbor_rods_xy):
            neighbor_rods = np.vstack([np.array(i) for i in it.chain(regular_neighbor_rods,periodic_neighbor_rods_y,periodic_neighbor_rods_x,periodic_neighbor_rods_xy)])
...
def change_rod_loc(rods,rod_to_chng,Lx,Ly):
    ...
    while new_rod_loc[0]<0 or new_rod_loc[0]>Lx:
        if new_rod_loc[0]<0:
            new_rod_loc[0] = new_rod_loc[0]+Lx
        elif new_rod_loc[0]>Lx:
            new_rod_loc[0] = new_rod_loc[0]-Lx
    while new_rod_loc[1]<0 or new_rod_loc[1]>Ly:
        if new_rod_loc[1]<0:
            new_rod_loc[1] = new_rod_loc[1]+Ly
        elif new_rod_loc[1]>Ly:
            new_rod_loc[1] = new_rod_loc[1]-Ly
    return new_rod_loc
...
def change_rod(rods,rod_to_chng,rod_l,Lx,Ly,sd_normal = 1,draw_only_allowed_angle = True):
    ...
    #draw new location (normal distribution, casue its isotropic)
    new_rod_loc = change_rod_loc(rods,rod_to_chng,Lx,Ly)
    while any((rods[:,0:2]==new_rod_loc).all(1)):
        new_rod_loc = change_rod_loc(rods,rod_to_chng,Lx,Ly)
...
#            return gd_stp, rod_to_chng
    ...
    if Lx>new_rod_loc[0]>0 and Ly>new_rod_loc[1]>0 and not(any((rods[:,0:2]==new_rod_loc).all(1))):
        ...
        if draw_only_allowed_angle:
            ...
            if new_rod_loc[0]<rod_l:
                periodic_rods_x = rods[Lx-rods[:,0]<rod_l].copy()
                periodic_rods_x[:,0] = periodic_rods_x[:,0] - Lx
            elif Lx-new_rod_loc[0]<rod_l:
                periodic_rods_x = rods[rods[:,0]<rod_l].copy()            
                periodic_rods_x[:,0] = periodic_rods_x[:,0] + Lx
            
            if len(periodic_rods_x):
                periodic_x_dist = distance.cdist([new_rod_loc],periodic_rods_x[:,0:2].tolist())
                mask = periodic_x_dist<rod_l
                periodic_neighbor_rods_x = periodic_rods_x[mask[0]]
#            except(UnboundLocalError):
#                pass
        
            if new_rod_loc[1]<rod_l:
                periodic_rods_y = rods[Ly-rods[:,1]<rod_l].copy()
                periodic_rods_y[:,1] = periodic_rods_y[:,1] - Ly
            elif Ly-new_rod_loc[1]<rod_l:
                periodic_rods_y = rods[rods[:,1]<rod_l].copy()
                periodic_rods_y[:,1] = periodic_rods_y[:,1] + Ly
           
            if len(periodic_rods_y):
                periodic_y_dist = distance.cdist([new_rod_loc],periodic_rods_y[:,0:2].tolist())
                mask = periodic_y_dist<rod_l
                periodic_neighbor_rods_y = periodic_rods_y[mask[0]]
#            except(UnboundLocalError):
#                pass
        
            if new_rod_loc[0]<rod_l and new_rod_loc[1]<rod_l:
                periodic_rods_xy = rods[np.logical_and(Lx-rods[:,0]<rod_l,Ly-rods[:,1]<rod_l)].copy()
                periodic_rods_xy[:,0] = periodic_rods_xy[:,0] - Lx
                periodic_rods_xy[:,1] = periodic_rods_xy[:,1] - Ly
            elif new_rod_loc[0]<rod_l and Ly-new_rod_loc[1]<rod_l:
                periodic_rods_xy = rods[np.logical_and(Lx-rods[:,0]<rod_l,rods[:,1]<rod_l)].copy()
                periodic_rods_xy[:,0] = periodic_rods_xy[:,0] - Lx
                periodic_rods_xy[:,1] = periodic_rods_xy[:,1] + Ly
            elif Lx-new_rod_loc[0]<rod_l and new_rod_loc[1]<rod_l:
                periodic_rods_xy = rods[np.logical_and(rods[:,0]<rod_l,Ly-rods[:,1]<rod_l)].copy()
                periodic_rods_xy[:,0] = periodic_rods_xy[:,0] + Lx
                periodic_rods_xy[:,1] = periodic_rods_xy[:,1] - Ly         
            elif Lx-new_rod_loc[0]<rod_l and Ly-new_rod_loc[1]<rod_l:
                periodic_rods_xy = rods[np.logical_and(rods[:,0]<rod_l,rods[:,1]<rod_l)].copy()
                periodic_rods_xy[:,0] = periodic_rods_xy[:,0] + Lx
                periodic_rods_xy[:,1] = periodic_rods_xy[:,1] + Ly
            
            if len(periodic_rods_xy):
                periodic_xy_dist = distance.cdist([new_rod_loc],periodic_rods_xy[:,0:2].tolist())
                mask = periodic_xy_dist<rod_l
                periodic_neighbor_rods_xy = periodic_rods_xy[mask[0]]
#            except(UnboundLocalError):
#                pass
            ...
            #regular neighbors
            dist = distance.cdist([new_rod_loc],rods[:,0:2].tolist())
            mask = dist<rod_l
            regular_neighbor_rods = rods[mask[0]]
...
if __name__ == '__main__':
    ...
    cwd = os.getcwd()
    if not os.path.exists(cwd+'/onsager_mc_periodic_graphs/'):
        os.makedirs(cwd+'/onsager_mc_periodic_graphs/')
    output_folder = cwd+'/onsager_mc_periodic_graphs/'
...
#        print(c)
        ...
        print('Lx' +str(Lx) + 'Ly' +str(Ly) + 'N'+ str(rods_n) + 'rl' + str(rod_l) + 'it'+str(iterations))
...
#        rods_list_periodic.append(rods)
